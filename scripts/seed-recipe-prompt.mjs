 ...
 * 初始化菜谱生成提示词到数据库
 *
 * 运行方式: node scripts/seed-recipe-prompt.mjs
 */
...
const SYSTEM_PROMPT = `﻿系统提示词 (System Prompt)
...
const USER_PROMPT = `﻿用户提示词 (User Prompt)
...
async function main() {
  console.log("🚀 开始初始化菜谱生成提示词...");
  console.log(`   DATABASE_URL: ${connectionString ? "已设置" : "未设置"}`);

  try {
    const result = await prisma.aIPrompt.upsert({
      where: { key: "recipe_generate" },
      update: {
        name: "菜谱生成",
    ...
    console.log("✅ 菜谱生成提示词已成功保存到数据库");
    console.log(`   Key: ${result.key}`);
    console.log(`   Name: ${result.name}`);
    console.log(`   System Prompt 长度: ${SYSTEM_PROMPT.length} 字符`);
    console.log(`   User Prompt 长度: ${USER_PROMPT.length} 字符`);
