...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!crowd) return NextResponse.json({ success: false, error: "不存在" }, { status: 404 });
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const existing = await prisma.tag.findFirst({ where: { id, type: "crowd" } });
    if (!existing) return NextResponse.json({ success: false, error: "不存在" }, { status: 404 });

    if (slug && slug !== existing.slug) {
      const conflict = await prisma.tag.findFirst({ where: { slug, type: "crowd", id: { not: id } } });
      if (conflict) return NextResponse.json({ success: false, error: "该 slug 已存在" }, { status: 400 });
    }

    const crowd = await prisma.tag.update({
      where: { id },
      data: {
        ...(name !== undefined && { name }),
        ...(slug !== undefined && { slug }),
    ...
    if (name !== undefined || slug !== undefined) {
      await syncCollectionForTag("crowd", id, { name, slug });
    }
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    await handleCollectionOnTagDelete("crowd", id);
