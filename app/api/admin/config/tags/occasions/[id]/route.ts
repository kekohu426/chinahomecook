...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const occasion = await prisma.tag.findFirst({
      where: { id, type: "occasion" },
      include: { translations: true },
    });

    if (!occasion) return NextResponse.json({ success: false, error: "不存在" }, { status: 404 });
...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    const existing = await prisma.tag.findFirst({ where: { id, type: "occasion" } });
    if (!existing) return NextResponse.json({ success: false, error: "不存在" }, { status: 404 });

    if (slug && slug !== existing.slug) {
      const conflict = await prisma.tag.findFirst({ where: { slug, type: "occasion", id: { not: id } } });
      if (conflict) return NextResponse.json({ success: false, error: "该 slug 已存在" }, { status: 400 });
    }

    const occasion = await prisma.tag.update({
      where: { id },
      data: {
        ...(name !== undefined && { name }),
        ...(slug !== undefined && { slug }),
    ...
    if (name !== undefined || slug !== undefined) {
      await syncCollectionForTag("occasion", id, { name, slug });
    }
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    await handleCollectionOnTagDelete("occasion", id);
