...
export async function generateMetadata({
  ...
  const metaTitleFallback = t("recipe.occasion.metaTitle", locale).replace(
    "{name}",
    name
  );
  const metaDescriptionFallback = t("recipe.occasion.metaDescription", locale).replace(
    "{name}",
...
export default async function OccasionPage({
  ...
  const translation = locales
    .map((loc) => occasion.translations.find((t) => t.locale === loc))
    .find(Boolean);
  const occasionNameRaw = translation?.name || occasion.name;
  const occasionName = isEn
    ? ensureEnglish(
        occasionNameRaw,
        ensureEnglish(titleFromSlug(slug), "Occasion")
      )
    : occasionNameRaw;
  ...
  const seo = (collection?.seo as SeoConfig) || undefined;
  const occasionNameDescription = isEn ? occasionName.toLowerCase() : occasionName;
  const headerTitle = isEn
    ? ensureEnglish(seo?.h1En, "") ||
      t("recipe.occasion.headerTitle", locale).replace("{name}", occasionName)
    : seo?.h1Zh ||
      t("recipe.occasion.headerTitle", locale).replace("{name}", occasionName);
  const headerSubtitle =
    (isEn ? ensureEnglish(seo?.subtitleEn, "") : seo?.subtitleZh) ||
    (occasionDescription ||
      t("recipe.occasion.headerSubtitle", locale).replace(
        "{name}",
        occasionNameDescription
      ));
              ...
              <div className="bg-white rounded-xl border border-lightGray p-5">
                <div className="flex flex-wrap gap-2">
                  {relatedOccasions.map((o) => {
                    ...
                    const relatedNameRaw = trans?.name || o.name;
                    const relatedName = isEn
                      ? ensureEnglish(
                          relatedNameRaw,
                          ensureEnglish(titleFromSlug(o.slug), "Occasion")
                        )
                      : relatedNameRaw;
