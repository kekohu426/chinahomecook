...
export async function generateMetadata({
  ...
  const translation = locales
    ...
    .find(Boolean);
  const nameRaw = translation?.name || method.name;
  const name = isEn
    ? ensureEnglish(nameRaw, ensureEnglish(titleFromSlug(slug), "Method"))
    : nameRaw;
  ...
  const metaTitleFallback = t("recipe.method.metaTitle", locale).replace(
    "{name}",
    name
  );
  const metaDescriptionFallback = t("recipe.method.metaDescription", locale).replace(
    "{name}",
...
export default async function MethodPage({
  ...
  const translation = locales
    .map((loc) => method.translations.find((t) => t.locale === loc))
    .find(Boolean);
  const methodNameRaw = translation?.name || method.name;
  const methodName = isEn
    ? ensureEnglish(
        methodNameRaw,
        ensureEnglish(titleFromSlug(slug), "Method")
      )
    : methodNameRaw;
  ...
  const seo = (collection?.seo as SeoConfig) || undefined;
  const methodNameDescription = isEn ? methodName.toLowerCase() : methodName;
  const headerTitle = isEn
    ? ensureEnglish(seo?.h1En, "") ||
      t("recipe.method.headerTitle", locale).replace("{name}", methodName)
    : seo?.h1Zh ||
      t("recipe.method.headerTitle", locale).replace("{name}", methodName);
  const headerSubtitle =
    (isEn ? ensureEnglish(seo?.subtitleEn, "") : seo?.subtitleZh) ||
    (methodDescription ||
      t("recipe.method.headerSubtitle", locale).replace(
        "{name}",
              ...
              <div className="bg-white rounded-xl border border-lightGray p-5">
                <div className="flex flex-wrap gap-2">
                  {relatedMethods.map((m) => {
                    ...
                    const relatedNameRaw = t?.name || m.name;
                    const relatedName = isEn
                      ? ensureEnglish(
                          relatedNameRaw,
                          ensureEnglish(titleFromSlug(m.slug), "Method")
                        )
                      : relatedNameRaw;
