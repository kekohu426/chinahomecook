...
export async function generateMetadata({
  ...
  const translation = locales
    ...
    .find(Boolean);
  const nameRaw = translation?.name || scene.name;
  const name = isEn
    ? ensureEnglish(nameRaw, ensureEnglish(titleFromSlug(slug), "Scene"))
    : nameRaw;
  ...
  const metaTitleFallback = t("recipe.scene.metaTitle", locale).replace(
    "{name}",
    name
  );
  const metaDescriptionFallback = t("recipe.scene.metaDescription", locale).replace(
    "{name}",
...
export default async function ScenePage({
  ...
  const translation = locales
    .map((loc) => scene.translations.find((t) => t.locale === loc))
    .find(Boolean);
  const sceneNameRaw = translation?.name || scene.name;
  const sceneName = isEn
    ? ensureEnglish(
        sceneNameRaw,
        ensureEnglish(titleFromSlug(slug), "Scene")
      )
    : sceneNameRaw;
  ...
  const seo = (collection?.seo as SeoConfig) || undefined;
  const sceneNameDescription = isEn ? sceneName.toLowerCase() : sceneName;
  const headerTitle = isEn
    ? ensureEnglish(seo?.h1En, "") ||
      t("recipe.scene.headerTitle", locale).replace("{name}", sceneName)
    : seo?.h1Zh ||
      t("recipe.scene.headerTitle", locale).replace("{name}", sceneName);
  const headerSubtitle =
    (isEn ? ensureEnglish(seo?.subtitleEn, "") : seo?.subtitleZh) ||
    (sceneDescription ||
      t("recipe.scene.headerSubtitle", locale).replace(
        "{name}",
        sceneNameDescription
      ));
              ...
              <div className="bg-white rounded-xl border border-lightGray p-5">
                <div className="flex flex-wrap gap-2">
                  {relatedScenes.map((s) => {
                    ...
                    const relatedNameRaw = t?.name || s.name;
                    const relatedName = isEn
                      ? ensureEnglish(
                          relatedNameRaw,
                          ensureEnglish(titleFromSlug(s.slug), "Scene")
                        )
                      : relatedNameRaw;
